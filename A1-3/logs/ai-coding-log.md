...
- 사용자는 여러 페이지 대신 단일 페이지 안에서 본문 화면을 전환하는 방향을 요청했습니다.
- 최종 화면 구조는 `Home`, `AI 메뉴 추천`, `랭킹` 세 화면으로 정했습니다.
...
- 구현된 주요 파일은 다음과 같습니다.
  - `frontend/index.html`: 고정 헤더와 본문을 주입할 앱 shell
  - `frontend/views/*.html`: Home, AI 추천, 랭킹 본문 화면 fragment
...
- 화면은 단일 페이지에서 고정 헤더와 3개 본문 화면 fragment fetch 전환 구조로 구현합니다.
