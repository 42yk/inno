...
아래 이미지는 `A1-2/images/`에 저장된 실제 실행 화면입니다.

### 날짜 형식 오류

`-date` 값이 `YYYY-MM-DD` 형식이 아니면 사용법과 오류 메시지를 출력하고 종료합니다.

![날짜 형식 오류 실행 화면](images/20260000-parse-error.png)

### 장소 검색 실패 후 계속 진행

Kakao Local API에서 인증/권한 오류가 발생해도 맛집 데이터는 없음으로 처리하고 최종 리포트 생성을 계속 진행합니다.

![Kakao Local API 실패 처리 실행 화면](images/2026-07-07-403.png)

### 정상 실행

Gemini 추천, Kakao Local 맛집 검색, 최종 리포트 생성까지 완료된 실행 화면입니다.

![정상 실행 화면](images/20260830-success.png)