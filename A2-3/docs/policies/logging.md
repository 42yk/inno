...
`logging_config.py`가 애플리케이션 시작 시 한 번만 설정한다. 로그 디렉터리가 없으면 생성하며 중복 핸들러를 추가하지 않는다. `logs/`와 로그 백업 파일은 Git에 포함하지 않는다.

## 3. 출력 대상과 포맷

- 콘솔과 로그 파일에 같은 최소 레벨을 적용한다.
...
- 기본 설정에서 콘솔 핸들러와 로그 파일 순환 핸들러가 각각 하나만 생성된다.
