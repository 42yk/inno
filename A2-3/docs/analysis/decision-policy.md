...
| 신뢰도 | 근거 리뷰의 평균 감정 confidence × 100. 값이 없으면 중립값 50 | 10% |

```text
우선순위 점수 = 빈도×0.40 + 심각도×0.30 + 증가도×0.20 + 신뢰도×0.10
```

Confidence는 보정된 정답 확률이 아니라 모델의 자기평가 확신도다. 이 요소는 저신뢰 근거의 추가 검수를 유도하는 보조 신호이며 빈도·심각도 근거를 대체하지 않는다. 상세 규약은 [AI 프롬프트 설계와 출력 계약](prompt-design.md)을 따른다.
